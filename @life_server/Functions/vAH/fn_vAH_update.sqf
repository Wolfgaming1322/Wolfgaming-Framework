...
switch (param [0]) do {
	case 0: {
		...
		sleep 0.3;
		diag_log "------------- Auktionshaus Zeit aktualisiert -------------";
		[] spawn TON_fnc_vAH_init;
	};
	case 1: {
		_query = format["DELETE FROM ah WHERE id ='%1'",_id];
		[_query,1] call DB_fnc_asyncCall;
		diag_log format["Auktionshaus: GEGENSTAND ENTFERNT: %1",_query];
		[] spawn TON_fnc_vAH_init;
	};
	case 2: {
	...
	case 3: {
		...
		[_query,1] call DB_fnc_asyncCall;
		diag_log format["Auktionshaus: GEGENSTAND VERKAUFT: %1",_query];
	...
	case 4: {
		...
		[_query,1] call DB_fnc_asyncCall;
		diag_log format["Auktionshaus: GEGENSTAND HINZUGEFÜGT: %1",_query];
		[] spawn TON_fnc_vAH_init;
	};
};
