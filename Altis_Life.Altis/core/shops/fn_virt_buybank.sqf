if(getNumber(missionConfigFile >> "Life_Settings" >> "vitemshop_buy_bank") isEqualTo 0) then {
  hint "Diese Funktion muss von einem Admin Freigeschalten werden";
} else {
  #include "..\..\script_macros.hpp"
  /*
  File: fn_virt_buybank.sqf
  ...
  if ((lbCurSel 2401) isEqualTo -1) exitWith {hint localize "STR_Shop_Virt_Nothing"};
    ...
    if (!([_amount] call TON_fnc_isnumber)) exitWith {hint localize "STR_Shop_Virt_NoNum";};
      ...
      if (_diff <= 0) exitWith {hint localize "STR_NOTF_NoSpace"};
        ...
        if ((_price * _amount) > BANK && {!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") <= _price * _amount}}) exitWith {hint localize "STR_NOTF_NotEnoughMoney"};
          if ((time - life_action_delay) < 0.2) exitWith {hint localize "STR_NOTF_ActionDelay";};
            ...
            if ([true,_type,_amount] call life_fnc_handleInv) then {
              if (!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") >= _price}) then {
                ...
                localize "STR_Shop_Virt_UI_YourBANK"
                ...
                if (_action) then {
                  ...
                  if ((_price * _amount) > BANK) exitWith {[false,_type,_amount] call life_fnc_handleInv; hint localize "STR_NOTF_NotEnoughMoney";};
                    hint format [localize "STR_Shop_Virt_BoughtItem",_amount,(localize _name),[(_price * _amount)] call life_fnc_numberText];
                    BANK = BANK - _price * _amount;
                  };
                } else {
