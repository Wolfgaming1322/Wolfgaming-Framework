...
if (count playerInventoryArray == 1) then {
		_myInvListbox lbAdd format ["Du hast keine Gegenstände im Inventar oder Auktionshaus!"];		
	} else {
{
	_status = [_x,2,3,[3]] call BIS_fnc_param;
	switch (_status) do {
		case 0: {_stat = "zu verkaufen"}; //AH: for sale
		case 1: {_stat = "Abgelaufen"}; //AH: expired
		case 2: {_stat = "VERKAUF SOLLTE NICHT SICHTBAR SEIN"}; //sold
		case 3: {_stat = "im eigenen Inventar"}; //MY INV
	};
	...
	_index = _index + 1;
	if (_item == "yolo") then {
			_myInvListbox lbAdd format ["--------------------Auktionshaus Inventar-----------------------"];
	...
	switch (_type) do {
		case "imenu": {
			...
			_time = round (_time / 3);
			if (_time <= 0) then {_price = 0; if (_status == 1) then {_toDel = round(_time + 216); _timeDisplay = format["- Wird in %1 Std. entfernt",_toDel]} else {_timeDisplay = "";};} else {_timeDisplay = format["- Läuft in %1 Std. ab",_time]};
