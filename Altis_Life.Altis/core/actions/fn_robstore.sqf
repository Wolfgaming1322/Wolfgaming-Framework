...
if (side _robber != civilian) exitWith {
    hint "You can not rob this Gas Station!"
};
if (_robber distance _shop > 5) exitWith {
    hint "You need to be within 5m of the cashier to rob him!"
};
if !(_kassa) then {
    _kassa = 1000;
};
if (_rip) exitWith {
    hint "Robbery already in progress!"
};
...
if (_kassa == 0) exitWith {
    hint "There is no cash in the register!"
};
_rip = true;
...
if (_rip) then {
    ...
    do {
        ...
        "Marker200"
        setMarkerText "!ATTENTION! robbery !ATTENTION!";
        "Marker200"
